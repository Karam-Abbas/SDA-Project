...
public:
    ...
    vector<Question> get_question_list()
    {
        return questions;
    }
    string get_ID() const
    ...
    void Questions_printer()
    {
        for (unsigned int i = 0; i < questions.size(); i++)
        {
            cout << "ID: " << questions[i].getQuestionID() << " Question:" << questions[i].get_question_description() << endl;
        }
    }
};
...
public:
    ...
    vector<Evaluation *> get_evaluations_list()
    {
    ...
    cout << "Evaluations under course " << course_id << ":\n";
